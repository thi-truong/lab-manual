...
See [**README.md**](kamaehu_rayshader/README.md) for this tutorial. I have a couple files downloaded from [GEBCO Gridded Bathymetry Data](https://download.gebco.net/) in the "[**data**](/kamaehu_rayshader/data/)" folder.
...
```{r}
#describe("kamaehu_rayshader/data/gebco24_kamaehu.tif")

gebco24_kamaehu_info <- capture.output(describe("kamaehu_rayshader/data/gebco24_kamaehu.tif"))
```
...
```{r}
kamaehu_raster <- terra::rast("kamaehu_rayshader/data/gebco24_kamaehu.tif")
```
...
Read [**the csv file**](kamaehu_rayshader/data/truong2023_kamaehu_data.csv), which is compiled literature data from my PhD dissertation.

```{r}
# read csv file, keep column names
kamaehu_data <- read_csv("kamaehu_rayshader/data/truong2023_kamaehu_data.csv",
                         col_names = TRUE) 
```
...
```{r}
kamaehu_r <- terra::rast("kamaehu_rayshader/data/gebco24_kamaehu.tif")
```
...
![](kamaehu_rayshader/tutorial-images//clipboard-1945740990.png)
...
Get original extent from the raster package. Record the vector into 4 variables for xmin, xmax, ymin, ymax. It is easy for me to confuse myself among naming them as latitude vs. longitude and min vs. max, so I peresonally check this a few times to verify.
...
Define a new bounding box and set a custom amount to remove on each edge. I just set them all the same for convenience, even though I know I am technically taking more of longitude than latitude). Use the `convert_coords()` function to define a new cropped bounding box, which keeps the crs. I found the function online but did not document the source (apologies!)
...
## Map 1: Classic Bathymetry (2-D) {#map-1-classic-bathymetry-2-d}
...
# save as a png
render_snapshot(filename = "kamaehu_rayshader/outputs/image/kamaehu_bathymetry_contours.png")

# save as a video with 360 rotation
render_movie(filename = "kamaehu_rayshader/outputs/video/kamaehu_bathymetry_contours.mp4")
```
...
![](kamaehu_rayshader/tutorial-images/kamaehu_bathymetry_contours_large.gif){fig-alt="gif of rotating 3d bathymetry of kamaʻehuakanaloa seamount"}
...
See [**README.md**](kamaehu_rayshader/README.md) for this tutorial.
